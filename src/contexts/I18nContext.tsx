'use client'

import React, { createContext, useContext, useState } from 'react'

type Locale = 'en' | 'zh-CN'

type Dictionary = {
    [key in Locale]: {
        [key: string]: string
    }
}

const dictionaries: Dictionary = {
    'en': {
        'site.title': 'DoDoo Daily',
        'footer.copyright': 'copyright @{year} by DoDoo Daily. All rights reserved.',
        'menu.tasks': 'Tasks',
        'menu.emotions': 'Emotions',
        'menu.gallery': 'Gallery',
        'menu.journal': 'Journal',
        'menu.shop': 'Shop',
        'hud.coins': 'Coins',
        'hud.goldStars': 'Task Gold Stars',
        'hud.purpleStars': 'Art Purple Stars',
        'hud.penalties': 'Anger Penalties',
        'hud.parentMode': 'Parent Mode',
        'hud.familyTree': 'Family Tree',
        'hud.logout': 'Logout',
        'system.closed': 'System Closed',
        'system.closedDesc': 'The system is currently undergoing maintenance or has been closed by the parent.',
        'login.title': 'Welcome to DoDoo Daily!',
        'login.parent': 'Parent',
        'login.child': 'Child',
        'login.back': 'Back',
        'login.pinPlaceholder': 'PIN Code',
        'login.clickToContinue': 'Click your avatar to continue',
        'login.nicknamePlaceholder': 'Your Nickname',
        'login.error.userNotFound': 'User not found. Check your nickname.',
        'login.back': 'Back',
        'login.rememberMe': 'Remember for 30 days',
        'login.loginButton': 'Login',
        'login.error.userNotFound': 'User not found',
        'login.error.invalidPin': 'Invalid PIN',
        'login.error.network': 'Network error',
        'login.firstLaunchPlaceholder': 'Enter your name to start...',
        'login.firstLaunchAvatar': 'Click to set avatar',
        'setup.title': 'Add Child Profile',
        'setup.uploadAvatar': 'Upload',
        'setup.avatarHint': 'Avatar',
        'setup.realName': 'Real Name',
        'setup.namePlaceholder': 'Child\'s Name',
        'setup.nickname': 'Nickname',
        'setup.errorNickname': 'Please enter nickname',
        'setup.nicknamePlaceholder': 'Nickname',
        'setup.gender': 'Gender',
        'setup.boy': 'Boy',
        'setup.girl': 'Girl',
        'setup.secret': 'Secret',
        'setup.birthDate': 'Birth Date (optional)',
        'setup.errorAvatar': 'Please upload an avatar',
        'setup.errorName': 'Please enter child\'s name',
        'setup.errorSubmit': 'Submit failed, please try again',
        'setup.errorNetwork': 'Network error, please try again',
        'setup.submit': 'Complete Setup & Enter',
        'setup.skip': 'Skip for now',
        'parent.title': 'Parent Dashboard',
        'parent.rewards': 'Manual Rewards',
        'parent.rewardsDesc': 'Add or remove Stars manually for the child.',
        'parent.penalties': 'Emotion Penalties',
        'parent.penaltiesDesc': 'Record anger outbursts or manage existing penalties.',
        'parent.family': 'Family Members',
        'parent.familyDesc': 'Add, remove or archive family accounts.',
        'parent.tasks': 'Task Verification',
        'parent.tasksDesc': 'Confirm and reward completed tasks from children.',
        'parent.profile': 'My Profile',
        'parent.profileDesc': 'Update your name and avatar.',
        'parent.settings': 'Security PIN',
        'parent.settingsDesc': 'Change your parent access PIN.',
        'parent.orders': 'Shop Orders',
        'parent.noOrders': 'No orders yet.',
        'parent.logs': 'Activity Logs',
        'parent.noLogs': 'No logs found',
        'parent.addChild': 'Add Child',
        'parent.addItem': 'Add New Item',
        'parent.manualAdjust': 'Reward / Adjustment',
        'parent.reason': 'Reason',
        'parent.currency': 'Coins',
        'parent.goldStars': 'Gold Stars',
        'parent.purpleStars': 'Purple Stars',
        'parent.angerPenalty': 'Anger Penalty',
        'parent.nickname': 'Nickname',
        'parent.birthDate': 'Date of Birth',
        'parent.gender': 'Gender',
        'parent.zodiac': 'Zodiac Sign',
        'parent.chineseZodiac': 'Chinese Zodiac',
        'parent.milestone': 'Milestone',
        'parent.milestoneTip': 'Markers as a high-value milestone and displayed on the timeline.',
        'parent.timeline': 'Timeline',
        'gender.male': 'Boy',
        'gender.female': 'Girl',
        'gender.other': 'Secret',
        'button.manage': 'Manage',
        'button.record': 'Record',
        'button.settings': 'Settings',
        'button.add': 'Add',
        'button.save': 'Save',
        'button.create': 'Create',
        'button.delete': 'Delete',
        'button.apply': 'Apply',
        'button.cancel': 'Cancel',
        'button.ship': 'Ship',
        'button.complete': 'Complete',
        'button.remarks': 'Remarks',
        'button.next': 'Next',
        'parent.profileUpdateSuccess': 'Profile updated successfully!',
        'parent.archivedMembers': 'Archived Members',
        'parent.hideArchived': 'Hide Archived',
        'parent.showArchived': '{count} Archived',
        'parent.confirmArchive': 'Archive?',
        'parent.confirmArchiveDesc': 'Archived members will no longer be able to log in, but their data will be preserved.',
        'parent.confirmDelete': 'Delete?',
        'parent.confirmDeleteDesc': 'This action is permanent and will remove all associated data for this child.',
        'parent.familySub': 'Manage family members and their daily progress.',
        'parent.noChildren': 'No active children. Click "Add Child" to start!',
        'parent.memberPin': 'PIN (Optional)',
        'parent.balanceLogs': 'Activity Balance',
        'family.member': 'Member',
        'family.memberName': 'Name',
        'family.relationship': 'Relationship',
        'family.namePlaceholder': 'e.g. David',
        'family.relationshipPlaceholder': 'e.g. Father, Mother...',
        'family.parent': 'Parent',
        'family.none': 'None (Root)',
        'family.addMember': 'Add Member',
        'family.editMember': 'Edit Member',
        'family.updateMember': 'Update Member',
        'family.addToTree': 'Add to Tree',
        'family.generations': 'Ancestry & Family',
        'family.growing': 'Watering the family tree...',
        'family.emptyForest': 'Empty Forest',
        'family.emptyForestSub': 'Start your family history by planting the first seed.',
        'family.startFirstRoot': 'Plant First Seed',
        'family.role.me': 'Me',
        'family.role.father': 'Father',
        'family.role.mother': 'Mother',
        'family.role.grandpa_p': 'Grandpa (Paternal)',
        'family.role.grandma_p': 'Grandma (Paternal)',
        'family.role.grandpa_m': 'Grandpa (Maternal)',
        'family.role.grandma_m': 'Grandma (Maternal)',
        'family.role.brother': 'Brother',
        'family.role.sister': 'Sister',
        'family.role.other': 'Other',
        'tasks.title': 'Tasks',
        'tasks.newTask': 'New Task',
        'tasks.noTasks': 'No tasks found',
        'tasks.noTasksSub': 'Click "New Task" to set a goal.',
        'tasks.noPendingTasks': 'No tasks waiting for approval.',
        'tasks.status.pendingParent': 'Waiting for Parent',
        'tasks.dailyLoop': 'Daily Loop',
        'tasks.monthlyLoop': 'Monthly Loop',
        'tasks.createGoal': 'Create Goal',
        'tasks.form.nameLabel': 'Goal / Task Name',
        'tasks.form.namePlaceholder': 'Read a book',
        'tasks.form.rewardLabel': 'Reward Stars',
        'tasks.form.recurringLabel': 'Daily Loop',
        'tasks.form.monthlyRecurringLabel': 'Monthly Loop',
        'tasks.assigned': 'Assigned',
        'emotions.title': 'Emotions',
        'emotions.recordBtn': 'Record It Now',
        'emotions.angerQuestion': 'Anger Outburst?',
        'emotions.recordDesc': 'Recording an outburst gives 1 Anger Penalty shape.',
        'emotions.history': 'History',
        'emotions.noRecords': 'No records. Great job keeping emotions balanced!',
        'emotions.resolved': 'Resolved',
        'emotions.penalty': 'Penalty',
        'emotions.form.reasonLabel': 'Optional Reason / Notes',
        'emotions.form.reasonPlaceholder': 'e.g. Got mad about dinner...',
        'emotions.form.confirm': 'Confirm',
        'gallery.title': 'Gallery',
        'gallery.upload': 'Upload / Photo',
        'gallery.newAlbum': 'New Album',
        'gallery.noAlbums': 'No Albums Yet',
        'gallery.noAlbumsSub': 'Click "New Album" to start your gallery!',
        'gallery.empty': 'Empty',
        'gallery.artworksCount': '{count} Artworks',
        'gallery.detail.empty': 'Empty Album',
        'gallery.detail.emptySub': 'Upload some art to this album!',
        'gallery.detail.notFound': 'Album Not Found',
        'gallery.detail.back': 'Back to Gallery',
        'parent.media': 'Media Management',
        'parent.mediaDesc': 'Manage uploaded images, voices, and documents.',
        'gallery.detail.exhibitionBadge': 'Poster',
        'gallery.detail.genPoster': 'Generate Poster',
        'gallery.form.titleLabel': 'Title',
        'gallery.form.titlePlaceholder': 'e.g. Summer Sky',
        'gallery.form.priceRmbLabel': 'Price RMB',
        'gallery.form.priceCoinsLabel': 'Price Coins',
        'gallery.form.isPublicLabel': 'Publish to Exhibition',
        'gallery.form.albumLabel': 'Select Album',
        'gallery.form.selectAlbum': 'Select an Album...',
        'gallery.form.noAlbumOption': 'No Album',
        'gallery.album.required': 'Please create an album first.',
        'gallery.form.fileLabel': 'Select File',
        'gallery.form.uploading': 'Uploading...',
        'gallery.form.submit': 'Submit / Publish',
        'gallery.album.nameLabel': 'Album Name',
        'gallery.album.namePlaceholder': 'e.g. 2024 Summer Works',
        'gallery.album.create': 'Create Album',
        'poster.exhibition': 'Art Exhibition',
        'poster.description': 'A piece of creative imagination from our little artist. Support their journey by collecting this artwork.',
        'poster.priceLabel': 'Collection Price',
        'poster.generating': 'Generating Masterpiece...',
        'poster.preview': 'Preview & Generate Poster',
        'poster.ready': 'Your Poster is Ready!',
        'poster.save': 'Save to Device',
        'poster.regen': 'Generate Again',
        'shop.title': 'Wish Shop',
        'shop.newWish': 'New Wish',
        'shop.buyWithCoins': 'Buy with Coins',
        'shop.buySuccess': 'Bought {name}! Check your HUD!',
        'shop.buyError': 'Purchase failed',
        'shop.management.title': 'Manual Rewards',
        'shop.management.add': 'Add Reward',
        'shop.management.edit': 'Edit Reward',
        'shop.management.unitPrice': 'Unit Price (Coins)',
        'shop.management.stock': 'Stock Quantity',
        'shop.management.delivery': 'Delivery Time',
        'shop.management.hours24': '24 Hours',
        'shop.management.days3': '3 Days',
        'shop.management.days7': '7 Days',
        'shop.management.days15': '15 Days',
        'shop.management.days30': '30 Days',
        'shop.form.addTitle': 'Add New Wish',
        'shop.form.nameLabel': 'Wish Name / Item',
        'shop.form.costLabel': 'Cost (Coins)',
        'shop.form.emojiLabel': 'Emoji Icon',
        'shop.form.confirm': 'Confirm Add',
        'shop.previewOnly': 'Preview Only',
        'journal.title': 'Journal',
        'journal.placeholder': 'What happened today?',
        'journal.post': 'Post',
        'journal.dailyPost': 'Daily Post',
        'journal.shareMoment': 'Share Moment',
        'journal.loading': 'Flipping through pages...',
        'journal.empty': 'No entries yet. Start writing!',
        'journal.copyright': '© DoDoo Daily Journal System',
        'common.loading': 'Loading...',
        'common.cancel': 'Cancel',
        'common.back': 'Back',
        'common.confirm': 'Confirm',
        'common.history': 'History',
        'parent.assignTask': 'Assign Task',
        'parent.adjustStats': 'Adjustment',
        'parent.previewChanges': 'Preview Changes',
        'parent.result': 'Result',
        'parent.archived': 'Archived',
        // Order management
        'order.status.pending': 'Pending',
        'order.status.completed': 'Completed',
        'order.status.refunded': 'Refunded',
        'order.action.confirm': 'Confirm',
        'order.action.refund': 'Refund',
        'order.remark.placeholder': 'Add a note for this order...',
        'order.myOrders': 'My Orders',
        'order.purchaseHistory': 'Purchase History',
        'order.loadingOrders': 'Loading orders...',
        'order.noOrdersDesc': 'Items you buy in the shop will appear here!',
        'order.goShopping': 'Go Shopping',
        // Shop management
        'shop.item.active': 'Active',
        'shop.item.inactive': 'Inactive',
        'shop.item.delete': 'Delete Reward',
        'shop.item.deleteConfirm': 'Confirm Delete',
        'shop.item.deleteDesc': 'Delete "{name}"? Existing orders are not affected and image files are kept.',
        'shop.item.deleteIrreversible': 'This action cannot be undone.',
        'shop.wishes.title': "Children's Wishes",
        'shop.wishes.pending': 'Pending',
        'shop.wishes.confirmed': 'Confirmed',
        'shop.wishes.addedToShop': 'In Shop',
        'shop.wishes.rejected': 'Rejected',
        'shop.wishes.confirm': 'Confirm',
        'shop.wishes.addToShop': 'Add to Shop',
        'shop.wishes.addToShopModal': 'Add to Shop',
        'shop.wishes.addToShopDesc': 'Add "{name}" to the reward shop with a coin cost.',
        'shop.wishes.coinCost': 'Coin Cost',
        'shop.wishes.confirmAdd': 'Confirm Add',
        'shop.wishes.alreadyAdded': 'Already added to shop',
        'shop.wishes.noWishes': 'No wishes yet.',
        'wish.myWishes': 'My Wishes',
        'wish.waitingForParents': 'Waiting for parents',
        'wish.loadingWishes': 'Loading wishes...',
        'wish.noWishes': 'No wishes yet',
        'wish.noWishesDesc': "Ask for something you'd like to see in the shop!",
        'wish.requested': 'Requested:',
        'wish.noDescription': 'No description provided.',
        'shop.management.productDetails': 'Product Details',
        'shop.management.photo': 'Photo',
        'shop.management.productName': 'Product Name',
        'shop.management.description': 'Description'
    },
    'zh-CN': {
        'site.title': 'DoDoo Daily',
        'footer.copyright': '© {year} DoDoo Daily. 保留所有权利。',
        'menu.tasks': '任务',
        'menu.emotions': '情绪',
        'menu.gallery': '画廊',
        'menu.journal': '日志',
        'menu.shop': '商店',
        'hud.coins': '金币',
        'hud.goldStars': '任务金星',
        'hud.purpleStars': '艺术紫星',
        'hud.penalties': '情绪惩罚',
        'hud.parentMode': '家长模式',
        'hud.familyTree': '家谱树',
        'hud.logout': '退出登录',
        'system.closed': '系统已关闭',
        'system.closedDesc': '系统目前正在维护或已被家长关闭，请稍后再试。',
        'login.title': '欢迎来到 DoDoo Daily！',
        'login.parent': '家长',
        'login.child': '儿童',
        'login.back': '返回',
        'login.pinPlaceholder': '请输入4位密码',
        'login.clickToContinue': '点击头像继续',
        'login.nicknamePlaceholder': '输入你的昵称/名称',
        'login.error.userNotFound': '找不到该成员，请检查名称是否正确',
        'login.back': '返回',
        'login.rememberMe': '记住登录状态',
        'login.loginButton': '登录',
        'login.error.userNotFound': '用户不存在',
        'login.error.invalidPin': '密码错误',
        'login.error.network': '网络连接失败',
        'login.firstLaunchPlaceholder': '请输入你的名字开始...',
        'login.firstLaunchAvatar': '点击设置头像',
        'setup.title': '添加孩子的信息',
        'setup.uploadAvatar': '上传头像',
        'setup.avatarHint': '头像',
        'setup.realName': '真实姓名',
        'setup.namePlaceholder': '孩子的名字',
        'setup.nickname': '昵称',
        'setup.errorNickname': '请输入昵称',
        'setup.nicknamePlaceholder': '小名 / 昵称',
        'setup.gender': '性别',
        'setup.boy': '男孩',
        'setup.girl': '女孩',
        'setup.secret': '保密',
        'setup.birthDate': '出生年月（可选）',
        'setup.errorAvatar': '请上传头像',
        'setup.errorName': '请输入孩子的名字',
        'setup.errorSubmit': '提交失败，请重试',
        'setup.errorNetwork': '网络错误，请重试',
        'setup.submit': '完成设置，进入首页',
        'setup.skip': '暂时跳过，稍后设置',
        'parent.title': '家长后台',
        'parent.rewards': '手动奖惩',
        'parent.rewardsDesc': '手动为孩子添加或减少星星。',
        'parent.penalties': '情绪记录',
        'parent.penaltiesDesc': '记录孩子的发脾气情况或管理惩罚。',
        'parent.family': '家庭成员管理',
        'parent.familyDesc': '添加、移除或归档家庭账户。',
        'parent.tasks': '任务核销',
        'parent.tasksDesc': '确认并对孩子提交的任务发放奖励。',
        'parent.profile': '个人资料',
        'parent.profileDesc': '修改您的昵称和头像。',
        'parent.settings': '安全密码',
        'parent.settingsDesc': '修改家长模式的访问 PIN 码。',
        'parent.orders': '商城订单',
        'parent.noOrders': '暂无订单。',
        'parent.logs': '活动日志',
        'parent.noLogs': '暂无日志',
        'parent.addChild': '添加孩子',
        'parent.addItem': '添加新商品',
        'parent.manualAdjust': '奖励与调整',
        'parent.reason': '理由',
        'parent.currency': '金币',
        'parent.goldStars': '金星',
        'parent.purpleStars': '紫星',
        'parent.angerPenalty': '情绪惩罚',
        'parent.nickname': '昵称',
        'parent.birthDate': '出生日期',
        'parent.gender': '性别',
        'parent.zodiac': '星座',
        'parent.chineseZodiac': '生肖',
        'parent.milestone': '大事记',
        'parent.milestoneTip': '标记为大事记将会在专属时间轴中突出展示。',
        'parent.media': '媒体管理',
        'parent.mediaDesc': '管理已上传的图片、声音和文档。',
        'parent.timeline': '成长时间轴',
        'gender.male': '男宝',
        'gender.female': '女宝',
        'gender.other': '保密',
        'button.manage': '管理',
        'button.record': '记录',
        'button.settings': '设置',
        'button.add': '添加',
        'button.save': '保存',
        'button.create': '创建',
        'button.delete': '删除',
        'button.apply': '应用',
        'button.cancel': '取消',
        'button.ship': '发货',
        'button.complete': '完成',
        'button.remarks': '备注',
        'button.next': '下一步',
        'parent.profileUpdateSuccess': '个人资料更新成功！',
        'parent.archivedMembers': '已归档成员',
        'parent.hideArchived': '隐藏已归档',
        'parent.showArchived': '显示 {count} 个归档',
        'parent.confirmArchive': '确定归档？',
        'parent.confirmArchiveDesc': '已归档成员将无法登录，但其数据将被保留。',
        'parent.confirmDelete': '确定删除？',
        'parent.confirmDeleteDesc': '此操作是永久性的，将删除该孩子的所有相关数据。',
        'parent.familySub': '管理家庭成员及其日常进度。',
        'parent.noChildren': '暂无成员，点击“添加成员”开始吧！',
        'parent.memberPin': '访问 PIN 码 (可选)',
        'parent.balanceLogs': '记账日志',
        'family.member': '成员',
        'family.memberName': '名字',
        'family.relationship': '关系',
        'family.namePlaceholder': '例如：张三',
        'family.relationshipPlaceholder': '选择或输入关系',
        'family.parent': '上级节点',
        'family.none': '无（作为根节点）',
        'family.addMember': '添加成员',
        'family.editMember': '编辑成员',
        'family.updateMember': '更新成员',
        'family.addToTree': '加入家族树',
        'family.generations': '家族世系',
        'family.growing': '正在茂盛生长...',
        'family.emptyForest': '空旷的森林',
        'family.emptyForestSub': '开始记录你的家族历史吧。',
        'family.startFirstRoot': '种下第一棵树',
        'family.role.me': '我',
        'family.role.father': '爸爸',
        'family.role.mother': '妈妈',
        'family.role.grandpa_p': '爷爷',
        'family.role.grandma_p': '奶奶',
        'family.role.grandpa_m': '姥爷/公公',
        'family.role.grandma_m': '姥姥/婆婆',
        'family.role.brother': '兄弟',
        'family.role.sister': '姐妹',
        'family.role.other': '其他',
        'tasks.title': '任务',
        'tasks.newTask': '新建任务',
        'tasks.noTasks': '暂无任务',
        'tasks.noTasksSub': '点击右上角添加新目标吧。',
        'tasks.noPendingTasks': '暂无待核销的任务。',
        'tasks.status.pendingParent': '待家长确认',
        'tasks.dailyLoop': '每日循环',
        'tasks.monthlyLoop': '每月循环',
        'tasks.createGoal': '创建目标',
        'tasks.form.nameLabel': '目标/任务名称',
        'tasks.form.namePlaceholder': '例如：读书30分钟',
        'tasks.form.rewardLabel': '金星奖励',
        'tasks.form.recurringLabel': '每日循环',
        'tasks.form.monthlyRecurringLabel': '每月循环',
        'tasks.assigned': '指派任务',
        'emotions.title': '情绪',
        'emotions.recordBtn': '记录惩罚',
        'emotions.angerQuestion': '又发脾气了?',
        'emotions.recordDesc': '记录一次情绪失控将计入 1 个图形惩罚。',
        'emotions.history': '历史记录',
        'emotions.noRecords': '没有记录，情绪管理得非常棒！',
        'emotions.resolved': '已化解',
        'emotions.penalty': '惩罚中',
        'emotions.form.reasonLabel': '备注/原因',
        'emotions.form.reasonPlaceholder': '例如：因为不能看电视生气',
        'emotions.form.confirm': '确认记录',
        'gallery.title': '画廊',
        'gallery.upload': '上传/拍照',
        'gallery.newAlbum': '新建画册',
        'gallery.noAlbums': '暂无画册',
        'gallery.noAlbumsSub': '点击新建画册开始您的个人展览！',
        'gallery.empty': '暂无内容',
        'gallery.artworksCount': '{count} 幅作品',
        'gallery.detail.empty': '画册为空',
        'gallery.detail.emptySub': '快去上传些作品吧！',
        'gallery.detail.notFound': '未找到该画册',
        'gallery.detail.back': '返回画廊',
        'gallery.detail.collected': '已收藏',
        'gallery.detail.exhibitionBadge': '海报作品',
        'gallery.detail.genPoster': '生成海报',
        'gallery.form.titleLabel': '画作名称',
        'gallery.form.titlePlaceholder': '例如：夏日的天空',
        'gallery.form.priceRmbLabel': '人民币售价',
        'gallery.form.priceCoinsLabel': '金币售价',
        'gallery.form.isPublicLabel': '公开展出',
        'gallery.form.albumLabel': '选择画册',
        'gallery.form.selectAlbum': '请选择一个画册...',
        'gallery.form.noAlbumOption': '不分配画册',
        'gallery.album.required': '请先创建一个画册。',
        'gallery.form.fileLabel': '选择图片',
        'gallery.form.uploading': '正在上传...',
        'gallery.form.submit': '发布作品',
        'gallery.album.nameLabel': '画册名称',
        'gallery.album.namePlaceholder': '例如：2024夏日特辑',
        'gallery.album.create': '创建画册',
        'poster.exhibition': '艺术展览',
        'poster.description': '来自我们小艺术家的创意想象。通过收藏这幅作品来支持他们的旅程。',
        'poster.priceLabel': '收藏价格',
        'poster.generating': '正在生成杰作...',
        'poster.preview': '预览并生成海报',
        'poster.ready': '您的海报已准备就绪！',
        'poster.save': '保存图片',
        'poster.regen': '重新生成',
        'shop.title': '愿望商店',
        'shop.newWish': '添加愿望',
        'shop.buyWithCoins': '金币兑换',
        'shop.buySuccess': '成功兑换 {name}！快看顶部状态栏！',
        'shop.buyError': '兑换失败',
        'shop.management.title': '手动奖励',
        'shop.management.add': '新增商品',
        'shop.management.edit': '编辑商品',
        'shop.management.unitPrice': '单价 (金币)',
        'shop.management.stock': '库存数量',
        'shop.management.delivery': '到货时间',
        'shop.management.hours24': '24小时',
        'shop.management.days3': '3天',
        'shop.management.days7': '7天',
        'shop.management.days15': '15天',
        'shop.management.days30': '30天',
        'shop.form.addTitle': '添加新愿望',
        'shop.form.nameLabel': '愿望名称',
        'shop.form.costLabel': '花费金币',
        'shop.form.emojiLabel': '图标',
        'shop.form.confirm': '确认添加',
        'shop.previewOnly': '仅供预览',
        'journal.title': '日志',
        'journal.placeholder': '今天发生了什么有趣的事？',
        'journal.post': '发布',
        'journal.dailyPost': '日常记录',
        'journal.shareMoment': '分享时刻',
        'journal.loading': '正在翻开日志本...',
        'journal.empty': '这里还是一片空白，开始记录第一篇日记吧！',
        'journal.copyright': '© DoDoo Daily 日志系统',
        'common.loading': '加载中...',
        'common.cancel': '取消',
        'common.back': '返回',
        'common.confirm': '确认',
        'common.history': '历史记录',
        'parent.assignTask': '分配任务',
        'parent.adjustStats': '奖惩调整',
        'parent.previewChanges': '预览变更',
        'parent.result': '结果',
        'parent.archived': '已归档',
        // 订单管理
        'order.status.pending': '待处理',
        'order.status.completed': '已完成',
        'order.status.refunded': '已退款',
        'order.action.confirm': '确认发货',
        'order.action.refund': '退款',
        'order.remark.placeholder': '为此订单添加备注...',
        'order.myOrders': '我的订单',
        'order.purchaseHistory': '购买记录',
        'order.loadingOrders': '正在加载订单...',
        'order.noOrdersDesc': '你在商店购买的商品会出现在这里！',
        'order.goShopping': '去逛逛',
        // 商品管理
        'shop.item.active': '上架',
        'shop.item.inactive': '下架',
        'shop.item.delete': '删除奖励',
        'shop.item.deleteConfirm': '确认删除',
        'shop.item.deleteDesc': '删除「{name}」？已购买的订单不受影响，图片文件保留。',
        'shop.item.deleteIrreversible': '此操作不可撤销。',
        'shop.wishes.title': '孩子的愿望申请',
        'shop.wishes.pending': '待处理',
        'shop.wishes.confirmed': '已确认',
        'shop.wishes.addedToShop': '已加入商店',
        'shop.wishes.rejected': '已拒绝',
        'shop.wishes.confirm': '确认',
        'shop.wishes.addToShop': '加入商店',
        'shop.wishes.addToShopModal': '加入商店',
        'shop.wishes.addToShopDesc': '将「{name}」加入奖励商店，并设置所需金币数。',
        'shop.wishes.coinCost': '所需金币',
        'shop.wishes.confirmAdd': '确认加入',
        'shop.wishes.alreadyAdded': '已加入商店',
        'shop.wishes.noWishes': '暂无愿望。',
        'wish.myWishes': '我的愿望',
        'wish.waitingForParents': '等待家长处理',
        'wish.loadingWishes': '正在加载愿望...',
        'wish.noWishes': '暂无愿望',
        'wish.noWishesDesc': '有什么想要的商品吗？向家长许个愿吧！',
        'wish.requested': '申请时间：',
        'wish.noDescription': '暂无描述。',
        'shop.management.productDetails': '商品详情',
        'shop.management.photo': '照片',
        'shop.management.productName': '商品名称',
        'shop.management.description': '商品描述'
    }
}

interface I18nContextProps {
    locale: Locale
    setLocale: (locale: Locale) => void
    t: (key: string, params?: Record<string, string>) => string
}

const I18nContext = createContext<I18nContextProps | undefined>(undefined)

export function I18nProvider({ children }: { children: React.ReactNode }) {
    const [locale, setLocale] = useState<Locale>(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('dodoo-locale') as Locale
            if (saved && ['en', 'zh-CN'].includes(saved)) {
                return saved
            }
            const browserLang = navigator.language
            if (browserLang && browserLang.toLowerCase().includes('zh')) {
                return 'zh-CN'
            }
        }
        return 'en'
    })

    const handleSetLocale = (newLocale: Locale) => {
        setLocale(newLocale)
        localStorage.setItem('dodoo-locale', newLocale)
    }

    const t = (key: string, params?: Record<string, string>) => {
        let text = dictionaries[locale][key] || key
        if (params) {
            Object.entries(params).forEach(([k, v]) => {
                text = text.replace(`{${k}}`, v)
            })
        }
        return text
    }

    return (
        <I18nContext.Provider value={{ locale, setLocale: handleSetLocale, t }}>
            {children}
        </I18nContext.Provider>
    )
}

export function useI18n() {
    const context = useContext(I18nContext)
    if (!context) {
        throw new Error('useI18n must be used within an I18nProvider')
    }
    return context
}
