            ...
            <header className="relative z-10 flex flex-col px-6 py-4 md:px-10 md:py-6 backdrop-blur-md bg-white/40 border-b border-white/50 shadow-sm gap-4">
                ...
                <div className="flex flex-col md:flex-row gap-4 w-full">
                    <form onSubmit={handleSearch} className="relative flex-1 group">
                        ...
                        <input 
                            type="text" 
                            placeholder="Find items in shelf..." 
                            value={search}
                ...
                <div className="max-w-7xl mx-auto flex flex-col gap-16 relative min-h-full">
                    ...
                    {items.length === 0 && !loading ? (
                        <div className="flex flex-col items-center justify-center p-20 text-slate-400">
                            ...
                            <p className="text-xl font-bold">Nothing on the shelves yet</p>
                            <p className="text-sm">Start by adding your first household item!</p>
                        </div>
                    ) : (
                            ...
                            {items.map((item, idx) => (
                                ...
                                >
                                    ...
                                    <div className="mt-4 text-center">
                                        <h3 className="font-black text-slate-700 leading-tight line-clamp-2 px-2">{item.name}</h3>
                                        <div className="flex flex-wrap justify-center gap-1 mt-1">
                                          {JSON.parse(item.tags).map((tag: string) => (
                                            <span key={tag} className="text-[10px] bg-amber-100 text-amber-600 px-1.5 py-0.5 rounded-md font-bold uppercase tracking-tighter">
                                              {tag}
                                            </span>
                                          ))}
                                        </div>
                                    ...
                                    {/* SHELF BEAM DESIGN (Horizontal line under each row) */}
                                    <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 w-[120%] h-3 bg-gradient-to-b from-slate-300 to-slate-400 rounded-full shadow-[0_8px_10px_rgba(0,0,0,0.1)] z-[-1]" />
                                </motion.div>
                            ))}
                        </div>
                    )}
            ...
            <AnimatePresence>
                {activeLightboxIndex !== null && (
                    ...
                    >
                        ...
                        <div className="relative max-w-5xl w-full h-full flex flex-col md:flex-row items-center justify-center gap-8" onClick={e => e.stopPropagation()}>
                              <motion.div 
                                 layoutId={`item-${items[activeLightboxIndex].id}`}
                                 className="w-full md:w-2/3 max-h-[70vh] flex items-center justify-center text-white"
                               >
                                   {/* eslint-disable-next-line @next/next/no-img-element */}
                                   <img src={items[activeLightboxIndex].imageUrl} alt="preview" className="max-w-full max-h-full rounded-2xl shadow-2xl object-contain border border-white/10" />
                              </motion.div>
                           
                           <div className="w-full md:w-1/3 flex flex-col gap-4 text-white">
                                <h2 className="text-3xl font-black">{items[activeLightboxIndex].name}</h2>
                                {items[activeLightboxIndex].notes && (
                                  <p className="text-slate-400 bg-white/5 p-4 rounded-xl border border-white/5">{items[activeLightboxIndex].notes}</p>
                                )}
                                <div className="flex flex-wrap gap-2">
                                  {JSON.parse(items[activeLightboxIndex].tags).map((tag: string) => (
                                    <span key={tag} className="px-3 py-1 bg-amber-500 text-white rounded-full text-xs font-bold shadow-lg shadow-amber-500/20 uppercase tracking-widest">{tag}</span>
                                  ))}
                                </div>
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-8">Last Updated: {new Date(items[activeLightboxIndex].updatedAt).toLocaleDateString()}</p>
                           </div>
                        </div>
...
function ItemModal({ item, onClose, onSuccess }: { item: StorageItem | null, onClose: () => void, onSuccess: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto pt-10 pb-10">
            <motion.div
                initial={{ opacity: 0, scale: 0.9, y: 20 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden shadow-slate-900/40 my-auto"
            >
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-slate-50 border-none rounded-2xl p-4 focus:ring-2 focus:ring-amber-400 outline-none font-bold text-slate-700 h-[56px]"
                                    />
                                </div>
                            </div>
                        </div>
