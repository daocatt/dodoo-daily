        ...
        return (
            <div className="min-h-dvh flex flex-col items-center justify-center bg-[#e0f2fe] text-[#2c2416] p-4 text-center">
                ...
                <div className="z-10 bg-white/80 p-12 rounded-xl shadow-2xl backdrop-blur-md">
                    <h1 className="text-4xl font-black text-green-500 mb-4">{t('checkout.paymentSuccess')}</h1>
                    <p className="text-xl font-bold">{t('checkout.thankYou')}</p>
                    <p className="text-[#a89880] mt-4">{t('checkout.redirecting')}</p>
                </div>
            </div>
        )
    }
        ...
        <div className="min-h-dvh flex flex-col relative overflow-hidden bg-[#e0f2fe] text-[#2c2416] items-center justify-center p-4">
            ...
            <form onSubmit={handlePurchase} className="relative z-10 w-full max-w-sm bg-white/80 backdrop-blur-xl rounded-xl p-8 shadow-2xl border border-white/50 flex flex-col gap-6">
                <div className="text-center mb-4">
                    <h1 className="text-3xl font-black mb-2">{t('checkout.title')}</h1>
                    <p className="text-[#a89880] text-sm tracking-wide">{t('checkout.markCollector')}</p>
                </div>

                <div>
                    <label className="block text-sm font-bold text-[#6b5c45] mb-2">{t('checkout.nameLabel')}</label>
                    <input
                        type="text"
                        required
                        value={name}
                ...
                <div>
                    <label className="block text-sm font-bold text-[#6b5c45] mb-2">{t('checkout.phoneLabel')}</label>
                    <input
                        type="tel"
                        value={phone}
                ...
                <div className="bg-purple-100 p-4 rounded-xl border border-purple-200 mt-2">
                    <p className="text-sm text-purple-800 text-center font-bold">{t('checkout.simulatePayment')}</p>
                </div>
