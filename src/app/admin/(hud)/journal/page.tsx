...
export default function JournalPage() {
    ...
    const performDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/journal/${confirmDeleteId}`, { method: 'DELETE' })
            if (res.ok) {
    ...
    const getImages = (entry: JournalEntry) => {
    ...
    return (
        <div className="min-h-screen bg-[#D1CDBC] relative flex flex-col font-sans selection:bg-rose-100 selection:text-rose-900">
            <BausteinAdminNavbar 
                actions={
                    <Link href="/admin/journal/new" className="hardware-btn group">
                        <div className="hardware-well h-12 w-44 rounded-xl bg-[#DADBD4] shadow-well flex items-center justify-center relative overflow-hidden active:translate-y-0.5 transition-all">
                            <div className="hardware-cap absolute inset-1.5 bg-rose-500 group-hover:bg-rose-600 rounded-lg shadow-cap flex items-center justify-center gap-3 transition-all border border-rose-400/30">
            ...
            <main className="flex-1 flex flex-col items-center px-6 md:px-10 pt-6 pb-32 md:pt-10 max-w-7xl mx-auto w-full gap-8 md:gap-14 relative overflow-y-auto hide-scrollbar scroll-smooth">
                ...
                <div className="w-full flex flex-col lg:flex-row items-center justify-between gap-8 px-2">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6 w-full lg:w-auto">
                        ...
                        <div className="hardware-well p-1 bg-[#DADBD4] rounded-xl shadow-well flex gap-1 w-full sm:w-auto h-12">
                            <button
                                onClick={() => setActiveTab('feed')}
                                className={clsx(
                                    "flex-1 sm:w-32 h-full rounded-lg font-black text-[10px] uppercase tracking-tighter transition-all flex items-center justify-center gap-2 relative overflow-hidden",
                                    activeTab === 'feed' ? "bg-white text-rose-500 shadow-cap shadow-rose-900/5 ring-1 ring-black/5" : "text-slate-400 hover:text-slate-500"
                                )}
                            >
                            ...
                            <button
                                ...
                                className={clsx(
                                    "flex-1 sm:w-32 h-full rounded-lg font-black text-[10px] uppercase tracking-tighter transition-all flex items-center justify-center gap-2 relative overflow-hidden",
                                    activeTab === 'timeline' ? "bg-white text-rose-500 shadow-cap shadow-rose-900/5 ring-1 ring-black/5" : "text-slate-400 hover:text-slate-500"
                                )}
                            >
                            ...
                            <Link
                                href="/admin/journal/trash"
                                className="flex-1 sm:w-32 h-full rounded-lg font-black text-[10px] uppercase tracking-tighter transition-all flex items-center justify-center gap-2 text-slate-400 hover:text-slate-500"
                            >
                        ...
                        {/* Search Chassis */}
                        <form onSubmit={handleSearch} className="hardware-well p-1 bg-[#DADBD4] rounded-xl shadow-well flex-1 lg:w-72 h-12">
                            <div className="bg-white/80 rounded-lg px-4 flex items-center gap-3 border border-black/5 h-full focus-within:bg-white transition-all ring-inset focus-within:ring-2 focus-within:ring-rose-500/20">
                                ...
                                <div className="w-full baustein-panel bg-[#E2DFD2] rounded-[2.5rem] p-16 md:p-32 flex flex-col items-center justify-center text-center gap-8 border-4 border-[#C8C4B0] shadow-2xl">
                                    ...
                                    <Link href="/admin/journal/new" className="hardware-btn group">
                                         <div className="hardware-well h-12 w-48 rounded-xl bg-orange-500 shadow-well flex items-center justify-center relative active:translate-y-0.5 transition-all">
                                            <div className="hardware-cap absolute inset-1.5 bg-white group-hover:bg-orange-50 rounded-lg shadow-cap flex items-center justify-center gap-3 border border-black/5">
                                                <Plus className="w-4 h-4 text-orange-500" />
                                                <span className="text-[10px] font-black text-orange-500 uppercase italic tracking-tighter">{t('journal.createFirstLog')}</span>
                                            </div>
                                        </div>
                                    </Link>
                                </div>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="baustein-panel bg-white p-3 rounded-3xl border border-black/5 shadow-lg group-hover:shadow-2xl group-hover:-translate-y-2 transition-all duration-500 flex flex-col gap-4 relative overflow-hidden ring-1 ring-black/5 h-full">
                                                    {/* Media Sector - Clickable Identity */}
                                                    <div 
                                                        onClick={() => router.push(`/admin/journal/${entry.id}`)}
                                                        className="relative aspect-[3/2] rounded-2xl overflow-hidden hardware-well bg-slate-50 border border-black/5 shadow-inner cursor-pointer"
                                                    >
                                                        {images.length > 0 ? (
                                                            <Image
                                                                src={images[0]}
                                                            ...
                                                            <div className="flex flex-col items-center justify-center gap-2 opacity-10">
                                                                ...
                                                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest label-mono italic opacity-60">No Photo</span>
                                                            </div>
                                                        )}
                                                    ...
                                                    <div className="flex flex-col gap-3 px-1 pb-1">
                                                        <div 
                                                            onClick={() => router.push(`/admin/journal/${entry.id}`)}
                                                            className="flex flex-col gap-1 min-h-[4rem] cursor-pointer"
                                                        >
                                                            {entry.title ? (
                                                                <h3 className="text-base font-black text-slate-800 leading-none uppercase italic tracking-tighter truncate group-hover:text-rose-500 transition-colors">
                                                                    {entry.title}
                                                                </h3>
                                                            ) : null}
                                                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest label-mono italic mb-1">
                                                                {(function(d){ return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}` })(new Date(entry.createdAt))}
                                                            </span>
                                                            <p className={clsx(
                                                                "text-sm font-medium text-slate-600 line-clamp-2 leading-relaxed italic",
                                                        ...
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-1.5">
                                                                ...
                                                                >
                                                                    <div className="hardware-well w-8 h-8 rounded-lg bg-[#DADBD4] shadow-well flex items-center justify-center relative active:translate-y-0.5 transition-all">
                                                                ...
                                                                <button 
                                                                    onClick={(e) => {
                                                                        e.stopPropagation()
                                                                        router.push(`/admin/journal/${entry.id}?edit=true`)
                                                                    }}
                                                                    className="hardware-btn group"
                                                                >
                                                                    <div className="hardware-well w-8 h-8 rounded-lg bg-[#DADBD4] shadow-well flex items-center justify-center relative active:translate-y-0.5 transition-all">
                                                            ...
                                                            <div className="hardware-well w-8 h-8 rounded-lg bg-[#DADBD4] shadow-well flex items-center justify-center opacity-60 group-hover:opacity-100 transition-all active:translate-y-0.5">
            ...
            <ConfirmModal
                ...
                message={t('journal.deleteConfirm', { title: entries.find(e => e.id === confirmDeleteId)?.title || 'Untitled' })}
